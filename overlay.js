// overlay.js - Final fallback-safe cam with working preview logic copied from lobby
...
async function startPreviewCompatibleCamera() { try { const devices = await navigator.mediaDevices.enumerateDevices(); const cams = devices.filter(d => d.kind === "videoinput"); const mics = devices.filter(d => d.kind === "audioinput");
...
peer.on("stream", (stream) => { const box = document.getElementById(targetSeat); if (box) { const vid = box.querySelector("video"); if (vid) { vid.srcObject = stream; vid.play().catch(err => console.warn("Remote play failed", err)); } } });
