...
const name = localStorage.getItem("displayName") || user.displayName || user.email; const snap = await get(ref(db, `rooms/${roomCode}`)); const roomData = snap.val(); const playerCount = roomData.playerCount; const players = roomData.players; configureLayout(playerCount); updateNames(players); const camSuccess = await startPreviewCompatibleCamera(); if (!camSuccess) return; attachMyStream(mySeat, name); setupPeerSync(players); 
});
};
