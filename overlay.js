// overlay.js – Ultra-debugged, fallback-safe peer+name+camera sync
...
let selectedCamera = localStorage.getItem("selectedCamera") || ""; let selectedMic = localStorage.getItem("selectedMic") || ""; let localStream; let peers = {}; let peerConnected = {};

window.startOverlay = () => { console.log("▶️ Overlay starting...");

onAuthStateChanged(auth, async (user) => { if (!user || !roomCode || !mySeat) { alert("Missing user or room context"); return window.location.href = "/profile.html"; }
...
function updateNames(players) { for (const seat in players) { const box = document.getElementById(seat); if (box) { const label = box.querySelector(".name"); const pname = players[seat]?.name || seat; if (label) label.textContent = pname; console.log(✅ Seat ${seat} set to ${pname}); } } }
...
} catch (err) { console.error("❌ getUserMedia failed:", err); return false; } }

function attachMyStream(seat, name) { const box = document.getElementById(seat); if (!box) return; const vid = box.querySelector("video"); if (vid && localStream) { vid.srcObject = localStream; vid.muted = true; setTimeout(() => vid.play().catch(err => console.warn("play() failed", err)), 100); console.log(🎥 Local cam attached to ${seat}); } const label = box.querySelector(".name"); if (label) label.textContent = name; }
...
onChildAdded(ref(db, signals/${roomCode}), (snap) => { const seat = snap.key; if (seat === mySeat) return; onChildAdded(ref(db, signals/${roomCode}/${seat}), (sigSnap) => { const { from, signal } = sigSnap.val(); if (from === mySeat) return; if (!peers[seat]) createPeer(seat, false); peers[seat].signal(signal); console.log(📡 Signal received from ${seat}); }); }); }

function createPeer(targetSeat, initiator) { const peer = new SimplePeer({ initiator, trickle: false, stream: localStream });

peer.on("signal", (data) => { const payload = { from: mySeat, signal: data }; push(ref(db, signals/${roomCode}/${targetSeat}), payload); console.log(📤 Signal sent to ${targetSeat}); });

peer.on("stream", (stream) => { const box = document.getElementById(targetSeat); if (box) { const vid = box.querySelector("video"); if (vid) { vid.srcObject = stream; setTimeout(() => vid.play().catch(err => console.warn("Remote play failed", err)), 100); console.log(🎬 Remote stream received for ${targetSeat}); } } });

peer.on("error", (err) => console.error("❌ Peer error with", targetSeat, ":", err)); peers[targetSeat] = peer; }
