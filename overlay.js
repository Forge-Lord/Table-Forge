// overlay.js — Full revert to working player-name and local camera sync with SimplePeer relay
...
window.startOverlay = () => { onAuthStateChanged(auth, async (user) => { if (!user || !roomCode || !mySeat) { alert("Missing user or room context"); return window.location.href = "/profile.html"; }
