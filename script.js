// script.js (Final Create Room Patch)
...
async function createRoom() { const name = localStorage.getItem("displayName") || "Guest" + makeCode(3); const roomName = document.getElementById("roomName").value.trim(); const template = document.getElementById("template").value; const playerCount = parseInt(document.getElementById("playerCount").value); const roomId = "room-" + makeCode(); const roomRef = ref(db, rooms/${roomId}); const seatMap = { 2: ["p1", "p2"], 3: ["p1", "p2", "p3"], 4: ["p1", "p2", "p3", "p4"] };
...
async function joinRoom() { const name = localStorage.getItem("displayName") || "Guest" + makeCode(3); const code = document.getElementById("roomCode").value.trim(); if (!code) return alert("Please enter a room code");
...
window.createRoom = createRoom; window.joinRoom = joinRoom; window.startGame = startGame; window.flipCamera = flipCamera;
